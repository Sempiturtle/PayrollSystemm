<x-guest-layout>
    ...
    <div class="space-y-6">
        <form method="POST" action="{{ route('login') }}" class="space-y-6">
            ...
            <!-- Email Address -->
            <div class="space-y-2">
                <label for="email" class="block text-[11px] font-bold text-slate-400 uppercase tracking-widest pl-1">Admin Identity</label>
                <div class="relative group">
                    ...
                    <input id="email" type="email" name="email" value="{{ old('email') }}" required autofocus 
                           class="block w-full pl-10 pr-4 py-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl text-sm font-bold text-slate-900 dark:text-slate-100 placeholder-slate-400 focus:ring-2 focus:ring-indigo-600/20 focus:border-indigo-600 transition-all outline-none saas-shadow-md" 
            ...
            <div class="space-y-2">
                <div class="flex items-center justify-between pl-1">
                    <label for="password" class="block text-[11px] font-bold text-slate-400 uppercase tracking-widest">Admin Passkey</label>
        ...
        <div class="relative">
            ...
            <div class="relative flex justify-center text-[10px] uppercase font-bold"><span class="bg-slate-50 dark:bg-slate-950 px-2 text-slate-500 tracking-widest">Employee Portal Access</span></div>
        </div>

        <div>
            <a href="{{ route('auth.google') }}" class="w-full flex items-center justify-center gap-3 py-4 px-4 border-2 border-slate-200 dark:border-slate-800 rounded-xl text-xs font-black text-slate-700 dark:text-slate-200 bg-white dark:bg-slate-900 hover:bg-slate-50 dark:hover:bg-slate-800 transition-all shadow-sm uppercase tracking-widest">
