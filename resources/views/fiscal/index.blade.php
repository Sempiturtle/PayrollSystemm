    ...
    <div class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            @foreach($metrics as $m)
                <div class="bg-white rounded-3xl p-6 border border-slate-100 shadow-sm overflow-hidden relative group hover:shadow-md transition-all">
                    <div class="flex items-center justify-between mb-4">
                        <div class="w-8 h-8 rounded-lg bg-{{ $m['color'] }}-50 flex items-center justify-center text-{{ $m['color'] }}-600">
                            <h3 class="text-xs font-bold uppercase tracking-widest">{{ $m['label'] }}</h3>
                        </div>
                    </div>
                    <div class="text-2xl font-extrabold text-slate-900 tracking-tight">₱ {{ number_format($m['value'], 2) }}</div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="bg-slate-900 rounded-3xl p-8 text-white shadow-xl relative overflow-hidden group">
                <div class="flex items-center justify-between mb-6">
                    <div>
                        ...
                        <div class="text-4xl font-extrabold tracking-tight">₱ {{ number_format($totals['gross'], 2) }}</div>
                    </div>
                    <div class="w-12 h-12 rounded-2xl bg-white/10 flex items-center justify-center backdrop-blur-md text-emerald-400">
            ...
            <div class="bg-emerald-600 rounded-3xl p-8 text-white shadow-xl relative overflow-hidden group">
                <div class="flex items-center justify-between mb-6">
                    <div>
                        <h3 class="text-xs font-bold text-emerald-200 uppercase tracking-widest mb-1">Total Net Take-Home</h3>
                        <div class="text-4xl font-extrabold tracking-tight">₱ {{ number_format($totals['net'], 2) }}</div>
                    </div>
                    <div class="w-12 h-12 rounded-2xl bg-white/20 flex items-center justify-center backdrop-blur-md text-white">
