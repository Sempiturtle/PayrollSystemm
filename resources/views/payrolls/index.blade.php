                                ...
                                <div class="flex items-center justify-center gap-1.5 opacity-40 group-hover:opacity-100 transition-opacity">
                                    ...
                                    @if(auth()->user()->isAdmin() && strtolower($payroll->status) === 'draft')
                                        <div x-data="{ adjOpen: false }" class="relative">
                                            <button @click="adjOpen = true" class="inline-flex items-center justify-center w-7 h-7 rounded border border-indigo-200 bg-indigo-50 text-indigo-600 hover:bg-indigo-500 hover:text-white transition" title="Manage Adjustments">
                                            ...
                                            <div x-show="adjOpen" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4">
                                                <div class="fixed inset-0 bg-slate-900/40 backdrop-blur-sm" @click="adjOpen = false"></div>
                                                <div class="bg-white rounded-xl shadow-2xl w-full max-w-lg relative p-5 border border-slate-100 max-h-[80vh] overflow-y-auto">
                                                    <div class="flex items-center justify-between mb-4">
                                                        <div>
                                                            <h3 class="text-sm font-bold text-slate-900 tracking-tight">Payroll Adjustments</h3>
                                                            <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">{{ $payroll->user->name }} &bull; {{ $payroll->period_start->format('M d') }} - {{ $payroll->period_end->format('M d, Y') }}</p>
                                                        </div>
                                                        <button @click="adjOpen = false" class="w-7 h-7 rounded-full bg-slate-100 text-slate-400 hover:bg-slate-200 flex items-center justify-center transition">
                                                    ...
                                                    @if($payroll->adjustments->count() > 0)
                                                        <div class="mb-4 space-y-2">
                                                            @foreach($payroll->adjustments as $adj)
                                                                <div class="flex items-center justify-between p-3 rounded-lg {{ $adj->type === 'bonus' ? 'bg-emerald-50 border border-emerald-100' : 'bg-rose-50 border border-rose-100' }}">
                                                                    <div>
                                                                        <div class="text-xs font-bold {{ $adj->type === 'bonus' ? 'text-emerald-700' : 'text-rose-700' }}">{{ $adj->description }}</div>
                                                                        <div class="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">{{ ucfirst($adj->type) }} {{ $adj->remarks ? '— '.$adj->remarks : '' }}</div>
                                                                    </div>
                                                                    <div class="flex items-center gap-2">
                                                                        <span class="text-sm font-black tabular-nums {{ $adj->type === 'bonus' ? 'text-emerald-600' : 'text-rose-600' }}">{{ $adj->type === 'bonus' ? '+' : '-' }}₱{{ number_format($adj->amount, 2) }}</span>
                                                                        <form action="{{ route('payroll.adjustments.destroy', $adj) }}" method="POST" onsubmit="return confirm('Remove this adjustment?')">
                                                                            @csrf
                                                                            @method('DELETE')
                                                                            <button class="w-6 h-6 rounded bg-white border border-slate-200 text-slate-400 hover:text-rose-600 hover:border-rose-200 flex items-center justify-center transition">
                                                    ...
                                                    @else
                                                        <div class="mb-4 py-6 text-center text-slate-300 text-[10px] font-bold uppercase tracking-widest italic">No adjustments applied</div>
                                                    @endif

                                                    {{-- Add Adjustment Form --}}
                                                    <div class="border-t border-slate-100 pt-4">
                                                        <div class="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-3">Add New Adjustment</div>
                                                        <form action="{{ route('payroll.adjustments.store', $payroll) }}" method="POST" class="space-y-3">
                                                            @csrf
                                                            <div class="grid grid-cols-2 gap-3">
                                                                <div>
                                                                    <label class="block text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Type</label>
                                                                    <select name="type" required class="w-full bg-slate-50 border-slate-100 rounded-lg text-xs focus:ring-indigo-500 py-2">
                                                                        <option value="deduction">Deduction</option>
                                                                        <option value="bonus">Bonus</option>
                                                                    </select>
                                                                </div>
                                                                <div>
                                                                    <label class="block text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Amount (₱)</label>
                                                                    <input type="number" name="amount" step="0.01" min="0.01" required class="w-full bg-slate-50 border-slate-100 rounded-lg text-xs focus:ring-indigo-500 py-2" placeholder="0.00">
                                                                </div>
                                                            </div>
                                                            <div>
                                                                <label class="block text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Description</label>
                                                                <input type="text" name="description" required maxlength="255" class="w-full bg-slate-50 border-slate-100 rounded-lg text-xs focus:ring-indigo-500 py-2" placeholder="e.g. Cash Advance, Performance Bonus">
                                                            </div>
                                                            <div>
                                                                <label class="block text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Remarks (Optional)</label>
                                                                <input type="text" name="remarks" maxlength="500" class="w-full bg-slate-50 border-slate-100 rounded-lg text-xs focus:ring-indigo-500 py-2" placeholder="Internal notes...">
                                                            </div>
                                                            <div class="flex gap-3 pt-1">
                                                                <button type="button" @click="adjOpen = false" class="flex-1 py-2.5 px-4 bg-slate-100 text-slate-600 rounded-lg font-bold text-xs hover:bg-slate-200 transition">Cancel</button>
                                                                <button type="submit" class="flex-1 py-2.5 px-4 bg-indigo-600 text-white rounded-lg font-bold text-xs shadow-lg shadow-indigo-100 hover:bg-indigo-700 transition">Apply Adjustment</button>
                                                            </div>
                                                        </form>
                                                    </div>
                                                </div>
