<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-slate-800 leading-tight">Presence Master List</h2>
    </x-slot>
    ...
    <div x-data="{ 
        ...
        <div class="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm space-y-3">
            <form action="{{ route('attendance.index') }}" method="GET" class="space-y-3">
                {{-- Row 1: Date filters --}}
                <div class="grid grid-cols-2 gap-3">
                    <div class="space-y-1">
                        <label class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">From</label>
                        <input type="date" name="date_from" value="{{ $filters['date_from'] ?? '' }}" class="w-full bg-slate-50 border-slate-100 rounded-xl text-xs focus:ring-indigo-500">
                    </div>
                    <div class="space-y-1">
                        <label class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">To</label>
                        <input type="date" name="date_to" value="{{ $filters['date_to'] ?? '' }}" class="w-full bg-slate-50 border-slate-100 rounded-xl text-xs focus:ring-indigo-500">
                    </div>
                </div>
                {{-- Row 2: Employee + Status --}}
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <select name="user_id" class="w-full bg-slate-50 border-slate-100 rounded-xl text-xs focus:ring-indigo-500">
                        <option value="">All Employees</option>
                        @foreach($employees as $emp)
                            <option value="{{ $emp->id }}" {{ ($filters['user_id'] ?? '') == $emp->id ? 'selected' : '' }}>{{ $emp->name }}</option>
                        @endforeach
                    </select>
                    <select name="status" class="w-full bg-slate-50 border-slate-100 rounded-xl text-xs focus:ring-indigo-500">
                ...
                <div class="flex items-center gap-2 flex-wrap">
                    <button type="submit" class="flex items-center gap-1.5 px-4 py-2 bg-slate-900 text-white rounded-xl hover:bg-slate-800 transition text-xs font-bold">
                    ...
                    <div class="ml-auto flex items-center gap-2">
                        <a href="{{ route('attendance.export', request()->all()) }}" class="flex items-center gap-1.5 px-3 py-2 bg-emerald-50 text-emerald-600 border border-emerald-100 rounded-xl font-bold text-xs hover:bg-emerald-100 transition">
                            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
                            <span class="hidden sm:inline">Export CSV</span>
                            <span class="sm:hidden">CSV</span>
                        </a>
                        <button type="button" @click="addModal = true" class="flex items-center gap-1.5 px-3 py-2 bg-indigo-600 text-white rounded-xl font-bold text-xs shadow-lg shadow-indigo-100 hover:bg-indigo-700 transition active:scale-[0.98]">
                            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path></svg>
                            <span class="hidden sm:inline">Add Manual Log</span>
                            <span class="sm:hidden">Add Log</span>
                        </button>
                    </div>
                </div>
            </form>
                            ...
                            <tr class="hover:bg-slate-50/50 transition relative group">
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        <div class="w-8 h-8 rounded-lg bg-slate-900 text-white flex items-center justify-center text-[10px] font-bold shrink-0">
        ...
        <div class="lg:hidden space-y-2">
            @forelse($logs as $log)
                @php
                    $statusColor = $log->status === 'Late' ? 'bg-rose-50 text-rose-600 border-rose-100' : 'bg-emerald-50 text-emerald-600 border-emerald-100';
                @endphp
                <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-4">
                    <div class="flex items-start justify-between gap-3 mb-3">
                        <div class="flex items-center gap-3">
                            <div class="w-9 h-9 rounded-lg bg-slate-900 text-white flex items-center justify-center text-[10px] font-bold shrink-0">
                                {{ strtoupper(substr($log->user->name ?? '?', 0, 1)) }}
                            </div>
                            <div>
                                <div class="text-sm font-bold text-slate-800">{{ $log->user->name ?? 'Unknown' }}</div>
                                <div class="text-[10px] text-slate-400 uppercase tracking-wider">{{ $log->user->employee_id ?? '' }}</div>
                            </div>
                        </div>
                        <span class="px-2 py-1 rounded text-[10px] font-bold border uppercase tracking-wider shrink-0 {{ $statusColor }}">
                            {{ $log->status }}
                        </span>
                    </div>
                    <div class="grid grid-cols-3 gap-2 text-center">
                        <div class="bg-slate-50 rounded-lg p-2">
                            <div class="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-0.5">Date</div>
                            <div class="text-xs font-bold text-slate-700">{{ $log->date->format('M d') }}</div>
                            <div class="text-[9px] text-slate-400 italic">{{ $log->date->format('D') }}</div>
                        </div>
                        <div class="bg-slate-50 rounded-lg p-2">
                            <div class="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-0.5">In</div>
                            <div class="text-xs font-bold text-emerald-600">{{ $log->time_in ?? '—' }}</div>
                        </div>
                        <div class="bg-slate-50 rounded-lg p-2">
                            <div class="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-0.5">Out</div>
                            <div class="text-xs font-bold text-slate-600">{{ $log->time_out ?? '—' }}</div>
                        </div>
                    </div>
                    <div class="flex justify-end gap-2 mt-3 pt-3 border-t border-slate-50">
                        <button @click="openEdit({{ $log->toJson() }})" class="flex items-center gap-1 px-3 py-1.5 text-indigo-600 hover:bg-indigo-50 rounded-lg text-xs font-bold transition">
                        ...
                        <form action="{{ route('attendance.destroy', $log) }}" method="POST" onsubmit="return confirm('Delete this log?')">
                            @csrf @method('DELETE')
                            <button type="submit" class="flex items-center gap-1 px-3 py-1.5 text-rose-500 hover:bg-rose-50 rounded-lg text-xs font-bold transition">
            ...
            @empty
                <div class="bg-white rounded-2xl border border-slate-100 p-12 text-center text-slate-400 italic text-sm">
        ...
        <div x-show="addModal" x-cloak class="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
            ...
            <div class="bg-white rounded-t-2xl sm:rounded-2xl shadow-2xl w-full sm:max-w-md relative p-5 border border-slate-100 max-h-[90vh] overflow-y-auto">
                <h3 class="text-lg font-bold text-slate-800 mb-5">Manual Entry</h3>
                <form action="{{ route('attendance.store') }}" method="POST" class="space-y-4">
                    ...
                    <div>
                        <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Override Status</label>
        ...
        <div x-show="editModal" x-cloak class="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
            ...
            <div class="bg-white rounded-t-2xl sm:rounded-2xl shadow-2xl w-full sm:max-w-md relative p-5 border border-slate-100 max-h-[90vh] overflow-y-auto">
                <h3 class="text-lg font-bold text-slate-800 mb-5">Adjust Presence</h3>
